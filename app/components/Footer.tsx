import { Link } from "react-router";

export default function Footer() {
    return (
        <footer className="bg-white/20 backdrop-blur-sm border-t border-white/30 mt-20">
            <div className="max-w-6xl mx-auto px-4 py-12">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-20">
                    <div className="mr-[100px] mx-auto">
                        <h3 className="text-2xl font-bold text-gray-800 mb-4">Resumind</h3>
                        <p className="text-gray-700 text-sm leading-relaxed">
                            AI-powered resume optimization to help you land your dream job. Get instant feedback and improve your ATS score. Never forget good resume might lack but luck always shines.
                        </p>
                    </div>

                    <div className="ml-[100px]">
                        <h4 className="text-lg font-semibold text-gray-800 mb-4">Features</h4>
                        <ul className="space-y-2 text-sm text-gray-700">
                            <li>AI Resume Analysis</li>
                            <li>ATS Score Rating</li>
                            <li>Job Description Matching</li>
                            <li>Instant Feedback</li>
                        </ul>
                    </div>

                    <div className="ml-[200px]">
                        <h4 className="text-lg font-semibold text-gray-800 mb-4">Quick Links</h4>
                        <div className="space-y-2 text-sm">
                            <Link to="/upload" className="block text-gray-700 hover:text-blue-600 transition-colors">
                                Upload Resume
                            </Link>
                            <Link to="/auth" className="block text-gray-700 hover:text-blue-600 transition-colors">
                                Login / Register
                            </Link>
                            <a href="https://www.reddit.com/user/Aditya_30" className="block text-gray-700 hover:text-blue-600 transition-colors">
                                Help & Support
                            </a>
                        </div>

                        <div className="mt-6">
                            <h4 className="text-lg font-semibold text-gray-800 mb-4">Connect</h4>
                            <div className="flex space-x-4">
                                <a
                                    href="www.linkedin.com/in/omkar-tiwari"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-gray-700 hover:text-blue-600 transition-colors"
                                    title="LinkedIn"
                                >
                                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                                    </svg>
                                </a>
                                <a
                                    href="https://x.com/adityasengaar"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-gray-700 hover:text-blue-600 transition-colors"
                                    title="X (Twitter)"
                                >
                                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z"/>
                                    </svg>
                                </a>
                                <a
                                    href="https://github.com/OmkarTiwari99"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-gray-700 hover:text-blue-600 transition-colors"
                                    title="GitHub"
                                >
                                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="border-t border-white/30 mt-8 pt-6 flex flex-col items-center space-y-4">
                    <p className="text-sm text-gray-600">
                        © 2025 Resumind. Made with ❤️ by Omkar Tiwari.
                    </p>
                    <div className="flex space-x-6 text-sm">
                        <a href="#" className="text-gray-600 hover:text-blue-600 transition-colors">Privacy Policy</a>
                        <a href="#" className="text-gray-600 hover:text-blue-600 transition-colors">Terms of Service</a>
                        <a href="tiwariomkar217@gmail.com" className="text-gray-600 hover:text-blue-600 transition-colors">Contact</a>
                    </div>
                </div>
            </div>
        </footer>
    );
}
